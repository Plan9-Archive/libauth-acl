...
typedef struct	AuthAcl		AuthAcl;
...
struct AuthAcl {
	int gstatus;	// current global status
	int status;		// current status
	char *fname;	// acl file name
	char *name;		// acl name
	int tusers;		// length of list
	char **unames;	// list
};
...
// ACL additions
extern AuthAcl*	acl_open(char*);
extern int		acl_write(AuthAcl*);
extern int		acl_close(AuthAcl*);
extern int		acl_check(AuthAcl*, char*);
extern int		checkacl(char*, char*);
